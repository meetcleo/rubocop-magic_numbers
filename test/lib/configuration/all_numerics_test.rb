...
module Custom
  module Configuration
    class AllNumerics < Minitest::Test
      def setup
        @config = RuboCop::Config.new('Custom/NoMagicNumbers' => { 'Enabled' => true, 'ForbiddenNumerics' => 'All' })
        # We detect all numerics, so this is used in tests to check for all
        # combinations of representations of integers and floats. The string
        ...
        integers = %w[10 1_0]
        number_string = ["'1'"]
        @matched_numerics = (floats + integers + number_string).combination(2).to_a
      end
